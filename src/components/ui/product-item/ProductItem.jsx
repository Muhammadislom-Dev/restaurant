...
const ProductItem = (props) => {
  ...
  const orderItem = (id) => {
    const index = order.findIndex((el) => el.id === id);
    const findProduct = products.find((el) => el.id === id);
    let newOredrs = [];
    if (index < 0) {
      newOredrs = [...order, { ...findProduct, quantiti: 1 }];
    } else {
      newOredrs = order.map((el) => {
        if (el.id === id) {
          return {
            ...el,
            quantiti: el.quantiti + 1,
          };
        }
    ...
    dispatch(addOrder(newOredrs));
  };

  return (
    <li className="product__list-item" key={props.id}>
      <div className="card">
        <div className="card_img">
          <ul className="card_list">
            <li className="card_item" onClick={() => orderItem(props.id)}>
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
            ...
            <li className="card_item">
              <svg
                ...
                xmlns="http://www.w3.org/2000/svg">
                <path
                  d="M14.5656 16.0022C10.5704 19.1858 4.98549 18.3049 2.02729 14.6536C-0.827886 11.1285 -0.64932 6.04312 2.44811 2.82456C5.64856 -0.500763 10.6802 -0.944681 14.3277 1.78313C15.6345 2.76025 16.6104 4.00335 17.246 5.50805C17.8853 7.02149 18.0795 8.59611 17.8578 10.2201C17.6368 11.8365 16.9918 13.2769 15.9423 14.6212C16.0272 14.6767 16.1178 14.7167 16.1827 14.7816C17.3365 15.9304 18.4859 17.0836 19.6397 18.2324C19.9132 18.5047 20.0493 18.8218 19.9682 19.2039C19.8071 19.9644 18.9018 20.2578 18.3211 19.7359C18.0489 19.4918 17.7985 19.2227 17.54 18.9642C16.5923 18.017 15.6451 17.0693 14.6973 16.1221C14.658 16.0846 14.6168 16.0497 14.5656 16.0022ZM15.9529 8.98634C15.9629 5.12656 12.8468 2.00603 8.97825 2.00041C5.12035 1.99479 2.00669 5.09035 1.98983 8.94763C1.97297 12.8168 5.08289 15.946 8.96326 15.9641C12.8055 15.9823 15.9429 12.8499 15.9529 8.98634Z"
                  fill="#3D3D3D"
                />
              </svg>
            </li>
          ...
          <img
            className="img"
            src={`https://restaurant.dipsag.uz/api/uploads/images/${props.image_src}`}
            alt={props.title_en}
          />
        </div>

        <div className="card_body">
          <h4 className="card_title">{props.title_en}</h4>
