...
const ProductFilter = ({ post, setPost }) => {
  ...
  const newFilterProduct = products.filter(
    (product) =>
      product.category.includes(post.activeCategory) &&
      product.price.slice(1) >= 10 &&
      product.price.slice(1) <= post.price * 1
  );
  ...
  return (
    <div className="filter__box">
      ...
      <div className="filter__box-size">
        <h3 className="size__name">Size</h3>
        ...
        <ul className="size__list">
          {size.map((item, i) => (
            <li className="size__list-item" key={i}>
              <span>{item}</span>
              <span>{`(${
                products.filter((product) => product.size === item).length
              })`}</span>
            </li>
          ))}
        </ul>
